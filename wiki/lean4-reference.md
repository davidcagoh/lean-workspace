...
## File Organization for Fast Builds

Lake rebuilds every file that changed AND every file that (transitively) imports it. File organization determines which builds are fast and which are forced-full.

### The three-level hierarchy

```
Level 0 — Pure math infrastructure
  No project-specific types. Imports only Mathlib.
  Examples: TorusIntegrals.lean, GronwallIntegral.lean, Lemmas.lean
  Build time: fast (isolated; changes don't cascade to Level 2)

Level 1 — Shared types + definitions
  Project-specific structures, shared helpers. Imports Mathlib + Level 0.
  Examples: Defs.lean (SSB), JEPA.lean core defs
  Build time: medium (all downstream files rebuild)

Level 2 — Theorems / main results
  One file per theorem cluster. Imports Level 0 + Level 1.
  Examples: Theorem1.lean, Theorem2.lean (SSB), MainTheorem.lean (JEPA)
  Build time: fast (changes don't cascade further)
```

### Why this matters for Aristotle cherry-picks

When Aristotle returns new infrastructure (e.g. `doubleFillSet`, `volume_doubleFillSet`), it goes in a Level 0 file. You can fast-check it without rebuilding the Level 2 main file:

```bash
lake build SimplicialLatentGeometry.TorusIntegrals   # ~60 s — catches type errors
# Only once clean:
lake build SimplicialLatentGeometry.SimplicialDetection  # ~5 min — full check
```

When Aristotle proves a theorem stub already scaffolded in a Level 2 file, only that file rebuilds.

### Existing project file maps

| Project | Level 0 (fast, ~60 s) | Level 1 | Level 2 (theorem files) |
|---|---|---|---|
| **simplicial** | `TorusIntegrals.lean` (1429 L), `DisjointTriangles.lean` (76 L) | `SimplicialDetection.lean`* | — |
| **JEPA** | `GronwallIntegral.lean` (136 L), `Lemmas.lean` (498 L), `LaurentHelpers.lean` (65 L), `OffDiagHelpers.lean` (37 L), `PDLowerHelpers.lean` (194 L), `BootstrapLemmas.lean` (552 L) | `JEPA.lean` (1988 L) | `MainTheorem.lean` (292 L) |
| **SSB** | — | `Defs.lean` (280 L) | `Theorem1.lean`, `Theorem2.lean`, `Theorem3.lean`, `Theorem4.lean` + strong forms (~100–275 L each) |

*`SimplicialDetection.lean` (4868 L) is currently both Level 1 and Level 2 — the main refactor opportunity if it grows further.

### Rules for new projects and new sections

1. **Pure math first.** If a set of lemmas needs only Mathlib (no project types like `CechSample`, `AOTree`, etc.), put them in their own Level 0 file from the start. Name it `*Helpers.lean`, `*Integrals.lean`, or `*Lemmas.lean`.

2. **One theorem cluster per Level 2 file.** SSB is the model. Each major result (`Theorem1`, `Theorem2`, …) in its own file importing `Defs.lean`. At ~100–300 lines each, these rebuild in seconds.

3. **Don't split prematurely.** A file under ~400 lines with stable content rarely needs splitting. The overhead (added imports, maintaining cross-file references, Aristotle prompt updates) is only worthwhile when build time is actually felt or the file is frequently edited.

4. **Aristotle awareness.** Aristotle jobs reference file names in their output. If you plan to split a file later, do it BEFORE submitting the job — otherwise the returned code targets the old file name and must be manually re-routed.

### Tradeoffs

| Concern | Fine-grained (many small files) | Coarse-grained (few large files) |
|---|---|---|
| Incremental build speed | Fast — only changed files rebuild | Slow — one large file = full re-elaboration |
| Circular imports | Impossible — requires upfront design | N/A |
| Aristotle job scoping | Smaller context = cheaper, more reliable | Larger file = more noise in prompt |
| Maintenance | More `import` lines; more files to track | Simpler navigation |
| Refactor risk | Each split is a structural change | Rare need to refactor |

**Verdict:** Use the three-level hierarchy for new projects. For existing large files, split only when: (a) the file exceeds ~1000 lines of active proof content, (b) a natural separation exists (pure math vs. project types), and (c) you're not mid-session with open Aristotle jobs.
